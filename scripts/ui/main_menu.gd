...
func _on_new_game() -> void:
    queue_free()
    # Route through MainBootstrap to show game intro before level 1
    var bootstrap := get_tree().root.find_child("MainBootstrap", true, false)
    if bootstrap and bootstrap.has_method("request_new_game"):
        bootstrap.request_new_game()
    elif LevelManager:
        Global.reset_game()
