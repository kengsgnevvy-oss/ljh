...
func _trigger_screen_shake(strength_factor: float = 1.0) -> void:
    var shake_intensity: float = clamp(strength_factor / 80.0, 0.01, 0.12)
    var shake_duration: float = clamp(strength_factor / 200.0, 0.05, 0.2)
    Global.apply_screen_shake(shake_intensity, shake_duration)
